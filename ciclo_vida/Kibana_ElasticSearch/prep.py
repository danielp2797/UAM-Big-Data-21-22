...
def compute_PSY(df, window=14):  # (pd.DataFrame, int)
	...
	for ind, value in enumerate(df.PressureBool):
		df.loc[ind, 'PSY_lag0'] = sum(df.PressureBool[ind-window:ind])/window
...
def compute_RSI(df, window=14):  # (pd.DataFrame, int)
	...
	for ind, value in enumerate(df.PressureBool):
		...
		df.loc[ind, 'RSI_lag0'] = 100 - 100/(1+(gain/loss))
...
if __name__ == '__main__':	
	...
	data = yf.download(tickers='BTC-USD', start='2016-01-01', end=today_date())
	...
	data['PriceVar'] = (data.Close-data.Open)/data.Open * 100
